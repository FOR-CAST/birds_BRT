...
invisible(sapply(X = list.files(file.path("R"), full.names = TRUE), FUN = source))
...
setPaths(modulePath = file.path(getwd(), "modules"), cachePath = file.path(getwd(), "cache"),
         inputPath = file.path(getwd(), "outputs/19MAR19"), 
         outputPath = file.path(getwd(), "outputs/14MAY19")) # Had to override... don't have time to redo the code up there
getPaths() # shows where the 4 relevant paths are
...
birdsNWT = list(
  ...
  isEDHZHIE <- FALSE
  if (isEDHZHIE){
    Edehzhie.url <- "https://drive.google.com/open?id=15n9BOtswKCJ81-us1u8Dbs0WT9f8bagq"
#"https://drive.google.com/open?id=1o2VamVILFx8SHqMCPC8bmjM8pkr5fsks"
