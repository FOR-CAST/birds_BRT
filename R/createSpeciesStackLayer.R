...
if (version == "reducedBAM"){
  ...
  spFromPredictors <- spFromPredictors[!grepl(pattern = paste(toRem, collapse = "|"),
                                        x = spFromPredictors)]
  ...
  if (length(missingSp) != 0){
    speciesLayerNamesMiss <- rbindlist(lapply(X = missingSp, FUN = function(sp){
      ...
      speciesDT <- data.table::data.table(modelLayer = whichLay,
                                          speciesName = sp)
      return(speciesDT)
    }))
  }
...
if(useOnlyUplandsForPrediction){
  ...
  if (any(!names(table(forestUplandRTM[], useNA = "ifany")) %in% c("0", "1", NA)))
    ...
    pixelGroupMap <- Cache(postProcess, x = pixelGroupMap,
  ...
  speciesLayerNames <- merge(speciesLayerNames, eqvTB, all.x = TRUE,
               by.x = "speciesName", by.y = "KNN") # Previous eqvTBred
...
if (length(toKeep) != 0){    # If we don't have the species in our simulations, toKeep will have length == 0
    ...
    # i.e., not NA and > 0
        originalSpp <- Cache(loadStaticLayers, fileURL = urlStaticLayers, # Add Cache when fun is ready
                         pathData = pathData,
                         studyArea = studyArea,
        ...
        originalSppPix <- data.table(pixelID = 1:ncell(originalSpp[[1]]),
    ...
    cohortWide <- dcast(data = cohortSummed, formula = pixelID ~ speciesCode,
    ...
    sppPixTbCohortReady <- merge(data.table(pixelID = 1:ncell(zeroedMap)),
                                 sppPixTbCohortReady, by = "pixelID",
                                 all.x = TRUE)
    setkey(sppPixTbCohortReady, "pixelID")
    zeroedMap <- setValues(x = zeroedMap,
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  speciesLays <- speciesLayerNames[grepl(pattern = "Species",
...
if (version %in% c(1:8, "6a")){
  originalSpeciesLayers <- Cache(prepInputStack, url = urlStaticLayer,
                                 alsoExtract = "similar",
                                 destinationPath = pathData,
                                 studyArea = studyArea,
                                 rasterToMatch = rasterToMatch)
} else {
  laysNeeded <- c(speciesLayerNames[["modelLayer"]][grepl(x = speciesLayerNames[["modelLayer"]],
                                                          pattern = "Species")],
                  ageLayerName)
  originalSpeciesLayers <- Cache(loadStaticLayers, fileURL = urlStaticLayers, # Add Cache when fun is ready
                                            pathData = pathData,
                                            studyArea = studyArea,
                                            rasterToMatch = rasterToMatch,
                                            Province = Province,
  ...
  if (length(groupLayNames) != 0){
    groupsLayers <- raster::stack(lapply(groupLayNames, function(layName){
      ...
      message(paste0("Building species group layers for ", layName, " (", specificGroup,
                     "). The following species are included: ",
                     paste(spInCurrentGroup, collapse = ", ")))
      layerNames <- speciesLayerNames[LandR %in% spInCurrentGroup, modelLayer]
      layerNames <- unique(gsub(pattern = "Landsc750", replacement = "Species",
                         x = layerNames))
  ...
  landcoverLayers <- Cache(loadStaticLayers, fileURL = urlStaticLayers, # Add Cache when fun is ready
                       pathData = pathData,
                       studyArea = studyArea,
                       rasterToMatch = rasterToMatch,
                       Province = Province,
  ...
  spLaysForLandscape <- unlist(lapply(strsplit(x = landscapeLayerNames,
                                    split = "Landsc750_"), `[[`, 2))
  tic("Landscape layers elapsed time: ")

  landscapeLays <- stack(lapply(spLaysForLandscape, focalWeight = fw750,
  ...
  stop(paste0(lay, " is not available to build Landscape layer. ",
  ...
  layersAvailable <- c(names(staticLayers),
                       names(speciesStack),
  ...
  if (length(missingLayersNames) != 0){
    message(crayon::yellow(paste0("There are missing layers. Completing ",
                                  "prediction stack with zeroed layers for: ")))
