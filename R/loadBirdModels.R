loadBirdModels <- function(birdsList, folderUrl, pathData, version) {
  ...
  if (version == "reducedBAM") {
    listModName <- strsplit(substrBoth(strng = reproducible::basename2(unlist(allModels)),
                                       howManyCharacters = 7, fromEnd = FALSE), split = "WB-")
    names(downloadedModels) <- unlist(lapply(listModName, '[[', 2))
  } else {
    names(downloadedModels) <- substrBoth(strng = reproducible::basename2(unlist(allModels)),
